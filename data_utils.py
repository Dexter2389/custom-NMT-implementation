...
def convert_to_indices(text, word2index, reverse=False, eos=False, sos=False):
    """ converts the given input to int values corresponding to the given word2index if set to True reverse the sequence. if true append eos. if true insert sos in the beginning."""
    unknown_words = set()
    text_inds = []
    ...
    for sentence in text:
        ...
        for word in sentence:
            if word in word2index.keys():
                sentence_inds.append(word2index[word])
            else:
                sentence_inds.append(word2index['<unk>'])
                unknown_words.update(word)

        if reverse:
            sentence_inds = list(reversed(sentence_inds))
        if eos:
            sentence_inds.append(word2index['</s>'])
        if sos:
            sentence_inds.insert(0, word2index['<s>'])
        text_inds.append(sentence_inds)

    return text_inds, unknown_words


def convert_to_words(sentence, index2word):
    """Back to words"""
    return [index2word[word] for word in sentence]
